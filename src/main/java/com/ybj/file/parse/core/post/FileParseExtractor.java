...
public class FileParseExtractor implements ParseStartHandler
{
    ...
    public void startParse(BaseParseFileInfo baseFileInfo) throws ParseException
    {
        ...
        if (reader == null) {
            throw new ParseException("不支持的文件类型，请选择正确的模板导入。");
        }
    ...
    public void processOne(ParsedRow parsedRow, ParseContext parseContext) throws RowHandleException
    {
        ...
        if (curRow == headRow)
        {
            ...
            readHead(parsedRow, parseContext);

            // 如果表头是最后一行，说明是一个只有表头的空表
            if (parsedRow.isLastRow()) {
                CellParseMessage error = new CellParseMessage("导入的文件中没有可以解析的数据。", parsedRow.getSheetIndex(),
                        parsedRow.getRowIndex());
                rowMsg.add(error);
                rowMsg.setHasError(true);
                throw new IllegalHeadException(rowMsg);
            }
            return;
        }
     ...
     * @return
     */
    protected RowParseMessage readHead(ParsedRow parsedRow, ParseContext parseContext)
    {
        ...
        parseContext.clearColumnFieldMap();
        parseContext.putAllColumnFieldMap(columnFieldMap);
        return null;
    }
